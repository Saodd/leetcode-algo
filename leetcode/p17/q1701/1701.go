...
/*
2021-03-25
执行用时：168 ms, 在所有 Go 提交中击败了87.50%的用户
内存消耗：15.6 MB, 在所有 Go 提交中击败了97.50%的用户
*/
func averageWaitingTime(customers [][]int) float64 {
	var queueFinish = 0
	var total = 0
	for _, customer := range customers {
		arrival, cost := customer[0], customer[1]

		// 无需等待，立即开始
		if arrival >= queueFinish {
			total += cost
			queueFinish = arrival + cost
			continue
		}

		// 需要等待
		queueFinish += cost
		total += queueFinish - arrival
	}
