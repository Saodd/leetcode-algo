...
func BuildTests() []Case {
	return []Case{
		{
			name: "示例1",
			args: args{listnode.Unmarshal([]int{1, 2, 3, 3, 4, 4, 5})},
			want: listnode.Unmarshal([]int{1, 2, 5}),
		},
		{
			name: "示例2",
			args: args{listnode.Unmarshal([]int{1, 1, 1, 2, 3})},
			want: listnode.Unmarshal([]int{2, 3}),
		},
		{
			name: "空链表",
			args: args{listnode.Unmarshal([]int{})},
			want: listnode.Unmarshal([]int{}),
		},
		{
			name: "易错点：尾部重复",
			args: args{listnode.Unmarshal([]int{1, 2, 2})},
			want: listnode.Unmarshal([]int{1}),
		},
	}
}
...
func Do(t *testing.T, solution Solution) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := listnode.Marshal(solution(tt.args.head))
			want := listnode.Marshal(tt.want)
